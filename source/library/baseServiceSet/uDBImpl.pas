...
type
  ...
  TPOSDBExtensionMethodPoxy = class;
  ...
  private
    FExtensionMethodPoxy: TPOSDBExtensionMethodPoxy;
    procedure checkPoxy;
...
procedure TPOSStatement.checkPoxy;
begin
  if not Assigned(FExtensionMethodPoxy) then
    FExtensionMethodPoxy := TPOSDBExtensionMethodPoxy.Create(Self, Connection.DatabaseName, Connection.SQLiteConn, Self.GetCriticalSection);
end;
...
begin
  ...
  Result := FExtensionMethodPoxy.DestoryMemDb;
...
constructor TPOSDBExtensionMethodPoxy.Create(AOwner: TComponent; const ADBFileName: string; ASQLiteConn: TSQLite3Connection; ACriticalSection: TCriticalSection);
begin
  inherited Create(AOwner);
  MemDb := nil;
...
//内存数据库相关////////////////////////
function TPOSDBExtensionMethodPoxy.CreateMemDb: Boolean;    //建一个空内存库（包含attack一个实体库）
var
  ermsg: PChar;
  strl: string;
begin
...
function TPOSDBExtensionMethodPoxy.DestoryMemDb: Boolean;   //销毁一个内存库
begin
  Result := False;
  if MemDb = nil then
  begin
...
function TPOSDBExtensionMethodPoxy.AttachDb: Boolean;
var
  ermsg: PChar;
  strl: string;
  tSql: string;
...
function TPOSDBExtensionMethodPoxy.MemDbExecSql(vSql: PChar): Boolean;   //内存库执行一条语句
var
  ermsg: PChar;
  strl: string;
begin
...
function TPOSDBExtensionMethodPoxy.MemDbExecSql(vSqlList: TStringList): Boolean;  //内存库批量执行语句
begin
  Result := MemDbExecSql(PChar(vSqlList.Text));
end;

function TPOSDBExtensionMethodPoxy.MemDbExecSqlOnLock(vSql: PChar; boResetConn: Boolean): Boolean;
...
function TPOSDBExtensionMethodPoxy.MemDbExecSqlOnLock(vSqlList: TStringList; boResetConn: Boolean): Boolean;
  //内存库在锁中执行语句 （和实体表交换数据时专用）
begin
