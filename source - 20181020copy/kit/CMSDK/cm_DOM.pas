...
type
  ...
  TCMSimpleNodeList = class
  private
    FList: TFPObjectList;
    function GetCount: Integer;
    function GetItem(Index: Integer): TCMSimpleNode;
    procedure SetCount(AValue: Integer);
    procedure SetItem(Index: Integer; AValue: TCMSimpleNode);
  public
    constructor Create;
    destructor Destroy; override;
    Procedure Clear;
    Function Add(ANode: TCMSimpleNode): Integer;
    Procedure Delete(Index: Integer);
    Function Remove(ANode: TCMSimpleNode): Integer;
    Function IndexOf(ANode: TCMSimpleNode): Integer;
    Procedure Insert(Index: Integer; ANode: TCMSimpleNode);
    property Count: Integer read GetCount write SetCount;
    property Items[Index: Integer]: TCMSimpleNode read GetItem write SetItem; default;
  end;
  ...
  private
    ...
    FKey: string;
    FValue: string;
    FParentNode: TCMSimpleNode;
    FChildNodeList: TCMSimpleNodeList;
  protected
    function GetChildCount: Integer;
    function GetFirstChild: TCMSimpleNode;
    function GetLastChild: TCMSimpleNode;
    function GetIndex: Integer;
    function GetPreviousSibling: TCMSimpleNode;
    function GetNextSibling: TCMSimpleNode;
  public
    constructor Create(const AKey: string; AParentNode: TCMSimpleNode=nil); overload;
    constructor Create(const AKey, AValue: string; AParentNode: TCMSimpleNode=nil); virtual; overload;
    destructor Destroy; override;
    //
    ...
    //
    property ParentNode: TCMSimpleNode read FParentNode;
    function FindNode(const ANodeKey: string): TCMSimpleNode;
    function HasChildNodes: Boolean;
    property ChildCount: Integer read GetChildCount;
    property FirstChild: TCMSimpleNode read GetFirstChild;
    property LastChild: TCMSimpleNode read GetLastChild;
    property ChildNodes: TCMSimpleNodeList read FChildNodeList;
    function AppendChild(NewChild: TCMSimpleNode): Integer;
    function RemoveChild(OldChild: TCMSimpleNode): Integer; overload;
    function RemoveChild(const OldChildKey: string): Integer; overload;
    //
    property Index: Integer read GetIndex;
    property PreviousSibling: TCMSimpleNode read GetPreviousSibling;
    property NextSibling: TCMSimpleNode read GetNextSibling;
  end;
...
function TCMSimpleNodeList.GetCount: Integer;
begin
  Result := FList.Count;
end;

function TCMSimpleNodeList.GetItem(Index: Integer): TCMSimpleNode;
begin
  Result := TCMSimpleNode(FList.Items[Index]);
end;

procedure TCMSimpleNodeList.SetCount(AValue: Integer);
begin
  FList.Count := AValue;
end;

procedure TCMSimpleNodeList.SetItem(Index: Integer; AValue: TCMSimpleNode);
begin
  FList.Items[Index] := AValue;
end;

constructor TCMSimpleNodeList.Create;
begin
  inherited Create;
  FList := TFPObjectList.Create(True);
end;

destructor TCMSimpleNodeList.Destroy;
begin
  FList.Free;
  inherited Destroy;
end;

procedure TCMSimpleNodeList.Clear;
begin
  FList.Clear;
end;

function TCMSimpleNodeList.Add(ANode: TCMSimpleNode): Integer;
begin
  Result := FList.Add(aNode);
end;

procedure TCMSimpleNodeList.Delete(Index: Integer);
begin
  FList.Delete(Index);
end;

function TCMSimpleNodeList.Remove(ANode: TCMSimpleNode): Integer;
begin
  Result := FList.Remove(aNode);
end;

function TCMSimpleNodeList.IndexOf(ANode: TCMSimpleNode): Integer;
begin
  Result := FList.IndexOf(aNode);
end;

procedure TCMSimpleNodeList.Insert(Index: Integer; ANode: TCMSimpleNode);
begin
  FList.Insert(Index, aNode);
end;
...
constructor TCMSimpleNode.Create(const AKey: string; AParentNode: TCMSimpleNode=nil);
begin
  Self.Create(AKey, '', AParentNode);
end;

constructor TCMSimpleNode.Create(const AKey, AValue: string; AParentNode: TCMSimpleNode=nil);
begin
  inherited Create;
  FParentNode := AParentNode;
  FKey := AKey;
  FValue := AValue;
  FChildNodeList := TCMSimpleNodeList.Create;
  //
...
destructor TCMSimpleNode.Destroy;
begin
  if Assigned(FParentNode) then
    Exit;
  //
...
function TCMSimpleNode.FindNode(const ANodeKey: string): TCMSimpleNode;
var
  i: Integer;
begin
  Result := nil;
  if ANodeKey = Self.Key then
    begin
      Result := Self;
      Exit;
    end;
  if Assigned(FChildNodeList) then
    begin
      for i:=0 to FChildNodeList.Count-1 do
        begin
          if FChildNodeList[i].Key = ANodeKey then
            begin
  ...
  if Assigned(FParentNode) and Assigned(FParentNode.ChildNodes) then
    begin
      for i:=0 to FParentNode.ChildNodes.Count-1 do
        begin
          if FParentNode.ChildNodes[i].Key = ANodeKey then
            begin
...
function TCMSimpleNode.HasChildNodes: Boolean;
begin
  Result := Assigned(FChildNodeList) and (FChildNodeList.Count > 0);
end;

function TCMSimpleNode.AppendChild(NewChild: TCMSimpleNode): Integer;
begin
  Result := -1;
  if Assigned(FChildNodeList) then
    begin
...
function TCMSimpleNode.RemoveChild(OldChild: TCMSimpleNode): Integer;
begin
  Result := -1;
  if Assigned(FChildNodeList) then
...
function TCMSimpleNode.RemoveChild(const OldChildKey: string): Integer;
var
  i: Integer;
begin
  Result := -1;
  if Assigned(FChildNodeList) then
    begin
      for i:=0 to FChildNodeList.Count-1 do
        begin
          if FChildNodeList[i].Key = OldChildKey then
            begin
...
function TCMSimpleNode.GetChildCount: Integer;
begin
  Result := 0;
  if Assigned(FChildNodeList) then
...
function TCMSimpleNode.GetFirstChild: TCMSimpleNode;
begin
  Result := nil;
...
function TCMSimpleNode.GetLastChild: TCMSimpleNode;
begin
  Result := nil;
  if GetChildCount > 0 then
...
function TCMSimpleNode.GetIndex: Integer;
begin
  Result := -1;
  if Assigned(FParentNode) then
...
function TCMSimpleNode.GetPreviousSibling: TCMSimpleNode;
begin
  Result := nil;
  if GetIndex > 0 then
    if Assigned(FParentNode) then
...
function TCMSimpleNode.GetNextSibling: TCMSimpleNode;
begin
  Result := nil;
  if Assigned(FParentNode) then
    if Assigned(FParentNode.ChildNodes) then
